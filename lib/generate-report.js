...
async function generateReport(options) {
    ...
    const suite = {
        ...
        reportName,
        customStyle: customStyle && fs.pathExistsSync(resolve(process.cwd(), customStyle))
...
=====================================================================================\n`);
        ...
        suite.features.forEach((feature) => {
            ...
            if (feature.metadata) {
                if (Array.isArray(feature.metadata)) {
                    ...
                    if (feature.metadata.device)
                        feature.device = feature.metadata.device;
        ...
        scenario.steps.forEach((step) => {
            if (step.embeddings !== undefined) {
                ...
                embeddings.forEach((embedding, embeddingIndex) => {
                    ...
                    else if (embedding.mime_type === 'video/webm' ||
     ...
     * @private
     */
    async function _createFeaturesOverviewIndexPage(suite) {
     ...
     * @private
     */
    async function _createFeatureIndexPages(suite) {
    ...
    async function _createCssFile(suite) {
